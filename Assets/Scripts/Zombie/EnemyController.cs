...
public class EnemyController : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        PrefabVariables prefabVariables = gameManager.GetComponent<PrefabVariables>();
        //accesing variables 
        ...
        //caching them
        healthText = prefabVariables._healthText;
        healthSlider = prefabVariables._healthSlider;
        scoreText = prefabVariables._scoreText;
        dropdownMenu = prefabVariables._dropdownMenu;
        player = prefabVariables._player;
        bloodEffectAnim = prefabVariables._bloodEffectAnim;

        gameOverClip = prefabVariables._gameOverClip;
        zombieDeathClip = prefabVariables._zombieDeathClip;
        playerDamageClip = prefabVariables._playerDamageClip;
        zombieDamageClip = prefabVariables._zombieDamageClip;
    ...
    {
        if (other.gameObject.TryGetComponent(out FirstPersonController playerController))
        {
           if (onPlayerDamaged == null) onPlayerDamaged += PlayerTakingHit;
    ...
    {
        if (other.gameObject.TryGetComponent(out FirstPersonController playerController) && !isKilled)
        {
            EnemyAttack();
        }
    }
    ...
    public async void PlayerTakingHit()
    {
        healthSlider.value -= zombieDamage;
        audioSource.PlayOneShot(playerDamageClip);
        bloodEffectAnim.SetBool("Hit", true);
        healthText.text = $"{healthSlider.value} / 100";

        await Task.Delay(attackTime);

        if (bloodEffectAnim != null) bloodEffectAnim.SetBool("Hit", false);
    }
    ...
    private void PlayerDeath()
    {
        ...
        //switch off moving scripts
        player.GetComponent<FirstPersonController>().enabled = false;

        //switch off shooting scripts
        if (WeaponController.weaponIndex == 1)
            FindObjectOfType<RifleController>().gameObject.SetActive(false);

        else
            FindObjectOfType<PistolController>().gameObject.SetActive(false);
    }
